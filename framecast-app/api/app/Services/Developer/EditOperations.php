...
class EditOperations
{
    ...
    public const REWRITE_MODES = ['shorten', 'expand', 'stronger_hook', 'more_punchy', 'more_educational', 'more_salesy', 'simplify', 'scarier', 'more_dramatic', 'more_casual', 'more_formal'];
    ...
    public static function catalogue(): array
    {
        return [
            ...
            'rewrite_scene' => ['scene' => true, 'family' => 'script_and_structure', 'spends' => false, 'description' => 'Rewrite a scene\'s script in a mode and apply it.', 'inputs' => ['scene_id' => 'int', 'mode' => implode('|', self::REWRITE_MODES)]],
            ...
            'generate_image' => ['scene' => true, 'family' => 'visuals', 'spends' => true, 'description' => 'Generate an AI still from the scene\'s visual prompt (and character, if set).', 'inputs' => ['scene_id' => 'int', 'model_key' => 'string?']],
    ...
    public static function rules(string $op): array
    {
        ...
        return match ($op) {
            ...
            'generate_image' => ['scene_id' => $sid, 'model_key' => ['nullable', 'string', 'max:40']],
    ...
    {
        return match ($op) {
            'regenerate_voice' => CreditService::ttsCostForEngine(RoutingTTSAdapter::engineFor((string) data_get($scene?->voice_settings_json, 'voice_id', GeminiVoices::DEFAULT_VOICE), [])),
            'generate_image' => app(ImageAdapterFactory::class)->generationCost($c['model_key'] ?? null, (bool) $scene?->character_id),
            'edit_image' => app(ImageAdapterFactory::class)->referenceGenerationCost(($c['model_key'] ?? null) ?: \App\Jobs\EditSceneImageJob::EDIT_MODEL),
            'animate' => (function () use ($c) {
                $seconds = ((int) ($c['duration_seconds'] ?? 5)) >= 8 ? 10 : 5;
                if ($c['tier'] === 'spokesperson') {
                    return (int) CreditService::spokespersonCost((float) $seconds);
                }
    ...
    public static function execute(string $op, Request $outer, Project $project, array $c): array|JsonResponse
    {
        ...
        return self::run(fn () => match ($op) {
            ...
            'rewrite_scene' => $sceneController()->rewrite($req(['mode' => $c['mode'], 'apply' => true]), $sid),
            'regenerate_voice' => $sceneController()->regenerateVoice($req([]), $sid, app(\App\Services\Generation\TTS\TTSAdapter::class)),
            'swap_visual' => $sceneController()->swapVisual($req(array_filter(['visual_asset_id' => $c['visual_asset_id'] ?? null, 'query' => $c['query'] ?? null, 'visual_type' => $c['visual_type'] ?? null], fn ($v) => $v !== null)), $sid),
            'generate_image' => $sceneController()->generateImage($req(array_filter(['model_key' => $c['model_key'] ?? null], fn ($v) => $v !== null)), $sid),
            ...
            'update_project' => $projectController()->update($req(array_filter(array_intersect_key($c, array_flip(['title', 'aspect_ratio', 'channel_id', 'brand_kit_id', 'music_asset_id', 'music_settings_json'])), fn ($v) => $v !== null), 'PATCH'), $project->getKey()),
