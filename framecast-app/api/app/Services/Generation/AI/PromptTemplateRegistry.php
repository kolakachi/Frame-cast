...
class PromptTemplateRegistry
{
    ...
    public function template(string $key): array
    {
        return match ($key) {
            ...
            'ugc_shot_plan' => [
                'system' => 'You are a direct-response UGC ad director. You turn a script into a shot plan for a short vertical ad in which one person talks to camera and the picture cuts away to the product in use. Return JSON only in this shape: {"segments":[{"kind":"on_camera|b_roll","script_text":"...","seconds":number,"visual_brief":"...","source":"upload|stock|generate"}],"reasoning":"one short sentence"}. Rules. The FIRST segment is always on_camera - a face is what stops the scroll. Cover the ENTIRE script: every word appears in exactly one segment, in order, unchanged. script_text is clean spoken narration only, never stage directions or shot notes. Cut to b_roll exactly where the words describe the product, a result, or a moment of use; stay on_camera for opinion, feeling and recommendation, because a claim needs a face behind it. Aim for 2 to 4 on_camera segments in total and never more than 4 - each one is billed separately, so a plan that cuts back to camera every sentence is a bad plan even when it reads well. Merge adjacent on_camera lines into one segment. visual_brief is required for b_roll and must be a concrete, filmable shot (close-up of the heated panel glowing inside the jacket), never an abstraction; leave it empty for on_camera. source: upload when the shot can only be the customer own footage - any app screen, UI, dashboard or real product interaction; stock for generic scene-setting; generate for a product shot that can be rendered from a still. When in doubt for software, choose upload. seconds must be greater than zero and should track the natural read time of script_text.',
                'user' => "Plan the shots for this UGC ad.\nProduct: {{product}}\nWhat the product is used on or in: {{context}}\nTarget length: about {{duration}} seconds.\nLanguage: {{language}}\nFootage the customer has already uploaded: {{available_footage}}\nScript:\n{{script_text}}",
            ],
            'hook_options' => [
