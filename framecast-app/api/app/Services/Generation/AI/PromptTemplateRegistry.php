    ...
    public function template(string $key): array
    {
        return match ($key) {
            ...
            'scene_visual_concepts' => [
                'system' => 'You are an art director for short-form vertical video. For each scene you receive, write ONE distinct, concrete visual concept an image model can paint: subject, setting, composition, lighting, mood. Every scene must differ visibly from the others — vary settings, framing (close-up/wide/detail/overhead), and subjects. Never reuse the same location or composition twice. Never include on-image text. Return JSON only: {"scenes":[{"order":number,"visual":"..."}]} with exactly one entry per input scene.',
