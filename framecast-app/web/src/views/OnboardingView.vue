...
const stepMeta = [
  {
    ...
    subtitle: "This helps us tailor scripts and visuals to your audience.",
  },
  {
    title: "What's your first video about?",
    subtitle:
  ...
  },
  {
    ...
    subtitle: "Review your setup and we'll get to work.",
  },
];
...
const audiogramBgs = [
  {
    key: "dark",
    label: "Dark",
    style: "linear-gradient(180deg, #0d0d1a 0%, #0a0a14 100%)",
  },
  { key: "black", label: "Black", style: "#000" },
  {
    key: "purple",
    label: "Purple",
    style: "linear-gradient(135deg, #1a0a2e 0%, #0d0d2b 50%, #14102a 100%)",
  },
  {
    key: "ocean",
    label: "Ocean",
    style: "linear-gradient(135deg, #0a1628 0%, #0d1f3c 50%, #0a0e1a 100%)",
  },
];
...
const selectedNiche = computed(() =>
  niches.value.find((n) => n.id === selectedNicheId.value)
);
const selectedSourceTypeOption = computed(
  () => sourceTypes.find((s) => s.key === sourceType.value) ?? null
);
const selectedVisualTypeOption = computed(
  () => visualTypes.find((v) => v.key === visualType.value) ?? visualTypes[0]
);
const selectedAspectRatioOption = computed(
  () =>
    aspectRatios.find((a) => a.value === aspectRatio.value) ?? aspectRatios[0]
);
const selectedVoiceOption = computed(
  () =>
    voices.value.find((v) => v.provider_voice_key === selectedVoiceKey.value) ??
    null
);
...
const availableStyles = computed(() => {
  ...
  return [...list]
    .map((s) => ({
      ...s,
      gradient:
        STYLE_PREVIEW_META[s.key]?.gradient ??
...
const selectedStyleOption = computed(
  () =>
    availableStyles.value.find((s) => s.key === selectedStyle.value) ??
    availableStyles.value[0] ??
    null
);
...
const contentPlaceholder = computed(() => {
  const map = {
    ...
    product_description:
      "Describe the product, its benefits, and target audience…",
  };
...
function restoreState() {
  try {
    ...
    visualType.value = s.visualType ?? "ai_images";
...
async function launch() {
  ...
  try {
    const payload = {
      title: selectedNiche.value
        ? `${selectedNiche.value.name} — First Video`
        : "My First Video",
      ...
      voice_settings_json: selectedVoiceKey.value
        ? { voice_id: selectedVoiceKey.value }
        : null,
      visual_style: selectedStyle.value,
      ai_broll_style:
        visualType.value === "ai_images" ? selectedStyle.value : null,
      image_generation_settings_json:
        visualType.value === "waveform"
          ? {
      ...
      <div v-else>
        ...
        <div v-if="step === 1">
          <div class="ob-niche-grid">
            <button
              ...
              :class="[
                'ob-niche-card',
                selectedNicheId === niche.id ? 'selected' : '',
              ]"
              type="button"
            ...
            <button
              :class="[
                'ob-niche-card ob-niche-card--other',
                selectedNicheId === null ? 'selected' : '',
              ]"
              type="button"
          ...
          <div class="ob-actions">
            <button class="ob-btn ob-btn-ghost" type="button" @click="skip">
              Skip setup
            </button>
        ...
        <div v-else-if="step === 2">
          ...
          <div class="ob-actions">
            ...
            <div class="ob-actions-right">
              <button class="ob-btn ob-btn-ghost" type="button" @click="next">
                Skip
              </button>
              <button
          ...
          <div v-if="visualType === 'ai_images'" class="ob-field">
            ...
            <div class="ob-style-grid">
              <button
                ...
                :class="[
                  'ob-style-card',
                  selectedStyle === s.key ? 'selected' : '',
                ]"
                type="button"
                @click="selectedStyle = s.key"
              >
                <span
                  class="ob-style-swatch"
                  :style="
                    s.sample_url
                      ? {
                          backgroundImage: `url(${s.sample_url})`,
          ...
          <div v-if="visualType === 'waveform'" class="ob-field">
            ...
            <div class="ob-ag-grid">
              <button
                ...
                :class="[
                  'ob-ag-card',
                  audiogramStyle === ws.key ? 'selected' : '',
                ]"
                type="button"
            ...
            <div class="ob-ag-colors">
              ...
              <label class="ob-ag-color-custom" title="Custom color">
                <input
                  type="color"
                  :value="audiogramColor"
                  @input="audiogramColor = $event.target.value"
                />
                <span>＋</span>
              </label>
            </div>
          ...
          <div class="ob-summary">
            <div class="ob-summary-row">
              ...
              <span class="ob-summary-val">{{
                selectedNiche ? selectedNiche.name : "General"
              }}</span>
            </div>
            <div class="ob-summary-row">
              ...
              <span class="ob-summary-val">{{
                selectedSourceTypeOption
                  ? selectedSourceTypeOption.label
                  : sourceType
              }}</span>
            </div>
            <div class="ob-summary-row">
              ...
              <span class="ob-summary-val">{{
                selectedVisualTypeOption.label
              }}</span>
            </div>
            <div v-if="visualType === 'ai_images'" class="ob-summary-row">
              <span class="ob-summary-key">AI Style</span>
              <span class="ob-summary-val">{{
                selectedStyleOption ? selectedStyleOption.label : selectedStyle
              }}</span>
            </div>
            <div class="ob-summary-row">
              <span class="ob-summary-key">Format</span>
              <span class="ob-summary-val"
                >{{ aspectRatio }} — {{ selectedAspectRatioOption.hint }}</span
              >
            </div>
            <div class="ob-summary-row">
              <span class="ob-summary-key">Voice</span>
              <span class="ob-summary-val">{{
                selectedVoiceOption ? selectedVoiceOption.name : "Default"
              }}</span>
            </div>
          </div>
          ...
          <div class="ob-actions">
            ...
            <div class="ob-actions-right">
              <button
                ...
                @click="launch"
              >
                {{ submitting ? "Creating…" : "Create my first video" }}
              </button>
            </div>
          </div>
        </div>
...
.ob-niche-card.selected .ob-niche-desc {
  color: var(--text-dim);
}
.ob-niche-card--other {
  border-style: dashed;
...
.ob-source-card.selected .ob-source-icon {
  background: rgba(255, 107, 53, 0.15);
  color: var(--accent);
}
.ob-source-icon :deep(svg) {
  width: 14px;
  height: 14px;
}
.ob-source-body {
...
.ob-visual-card:hover {
  border-color: var(--border-strong);
  background: var(--surface-2);
}
.ob-visual-card.selected {
  border-color: var(--accent);
  background: var(--accent-soft);
}
.ob-visual-icon {
...
.ob-visual-card.selected .ob-visual-icon {
  background: rgba(255, 107, 53, 0.15);
  color: var(--accent);
}
.ob-visual-icon :deep(svg) {
  width: 16px;
  height: 16px;
}
.ob-visual-body {
...
.ob-style-grid::-webkit-scrollbar {
  width: 4px;
}
.ob-style-grid::-webkit-scrollbar-track {
  background: transparent;
}
.ob-style-grid::-webkit-scrollbar-thumb {
  background: var(--border);
  border-radius: 4px;
}
.ob-style-card {
...
/* ── Voice picker (replaces the default <select>) ── */
.ob-picker-wrap {
  position: relative;
}
.ob-picker-trigger {
...
.ob-picker-trigger:hover {
  border-color: var(--border-strong);
}
.ob-picker-trigger-glyph {
  width: 32px;
  height: 32px;
  border-radius: 7px;
...
.ob-ratio-card.selected .ob-ratio-shape {
  background: var(--accent);
}
.r-9-16 {
  width: 12px;
