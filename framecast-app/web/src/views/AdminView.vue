...
const affiliateTotals = computed(() =>
  affiliates.value.reduce(
    (t, a) => ({
...
const affiliateOwing = computed(
  () => affiliates.value.filter((a) => (a.available ?? 0) > 0).length
);
...
async function loadAffiliates() {
  ...
  try {
    ...
    if (affiliateOpen.value) {
      const fresh = affiliates.value.find(
        (a) => a.id === affiliateOpen.value.id
      );
      if (fresh) affiliateOpen.value = fresh;
    }
  } catch (e) {
    affiliateError.value =
      e.response?.data?.error?.message ?? "Could not load affiliates.";
  } finally {
  ...
  try {
    if (affiliateTab.value === "bank") {
      ...
      const { data } = await api.get(`/admin/affiliates/${a.id}/conversions`, {
        params:
          affiliateTab.value === "outstanding" ? { filter: "unpaid" } : {},
      });
...
async function downloadStatement(
  ...
  try {
    ...
    link.href = url;
    link.download = payout
      ? `${a.code}-${payout.reference}.csv`
      : `${a.code}-outstanding.csv`;
    link.click();
...
async function confirmPayout() {
  ...
  try {
    const { data } = await api.post(
      `/admin/affiliates/${draft.affiliate.id}/payouts`,
      {
    ...
    await loadAffiliates();
    if (affiliateOpen.value?.id === draft.affiliate.id)
      await loadAffiliateDetail();
    ...
    affiliateError.value =
      e.response?.data?.error?.message ?? "Could not record that payment.";
  } finally {
...
async function voidPayout(a, payout) {
  const reason = window.prompt(
    `Void ${payout.reference} (${money(payout.total_amount)})?\n\nIts ${
      payout.sales_count
    } sale(s) go back to outstanding. Say why:`
  );
  if (!reason) return;
  try {
    await api.post(`/admin/affiliates/${a.id}/payouts/${payout.id}/void`, {
      reason,
    });
    await loadAffiliates();
    await loadAffiliateDetail();
    ...
    affiliateError.value =
      e.response?.data?.error?.message ?? "Could not void that payment.";
  }
}
...
async function setBankStatus(status) {
  ...
  if (status === "rejected") {
    reason = window.prompt(
      "Why are these details rejected? The affiliate sees this."
    );
    if (!reason) return;
  }
  try {
    await api.post(`/admin/affiliates/${a.id}/payment-details/verify`, {
      status,
      reason,
    });
    await loadAffiliateBank();
    ...
    affiliateError.value =
      e.response?.data?.error?.message ?? "Could not update that.";
  }
}
...
async function savePayoutStatus() {
  ...
  try {
    await api.post(
      `/admin/affiliates/${affiliateOpen.value.id}/payouts/${d.payout.id}/status`,
      {
        status: d.status,
...
async function regenerateAffiliateKey(a) {
  ...
  try {
    const { data } = await api.post(
      `/admin/affiliates/${a.id}/regenerate-key`,
      {}
    );
...
async function copyAffiliateSignIn(a) {
  ...
  try {
    ...
    setTimeout(() => {
      if (affiliateCopied.value === `signin-${a.id}`)
...
async function copyAffiliateLink(a) {
  try {
    ...
    setTimeout(() => {
      if (affiliateCopied.value === a.id) affiliateCopied.value = null;
    }, 1600);
  } catch {
...
const topbarTitles = {
  ...
  "client-workspaces": "Client Workspaces",
  videos: "All Videos",
  jobs: "Queue & Jobs",
  billing: "Billing & Spend",
  audit: "Audit Log",
...
function mailIncludeAll() {
  mailExcluded.value = new Set();
}
function mailExcludeAll() {
  mailExcluded.value = new Set(
    (mailRecipients.value?.recipients ?? []).map((r) => r.email)
  );
}
...
watch(mailSegment, async (seg) => {
  ...
  try {
    const res = await api.get("/admin/mail/recipients", {
      params: { segment: seg },
    });
...
async function loadMailHistory() {
  try {
    const res = await api.get("/admin/mail/history", {
      params: {
        page: mailHistoryPage.value,
        per_page: mailHistoryPerPage.value,
      },
    });
...
const mailDraftAudience = computed(() => {
  ...
  return {
    ...
    label: `the ${mailSegment.value} segment`,
  };
});
...
async function sendMail() {
  ...
  if (!mailSubject.value.trim() || !mailBody.value.trim()) {
    mailError.value = "Subject and message are required.";
    return;
  }
  if (!mailRecipientCount.value) {
    mailError.value = "No recipients.";
    return;
  }
  // Broadcasts get an explicit confirm step; a single reply-style mail doesn't need one.
  if (mailSegment.value !== "custom" && !mailConfirming.value) {
    mailConfirming.value = true;
    return;
  }
  ...
  try {
    const res = await api.post("/admin/mail/send", {
      segment: mailSegment.value,
      ...(mailSegment.value === "custom"
        ? { emails: mailCustomList.value }
        : {}),
...
async function loadModeration() {
  ...
  try {
    ...
    modCounters.value = res.data?.data?.counters ?? {
      total_24h: 0,
      unreviewed: 0,
      high_severity: 0,
    };
...
async function submitModReview() {
  ...
  try {
    const res = await api.patch(
      `/admin/moderation/events/${modSelectedEvent.value.event.id}`,
      modActionDraft.value
    );
    modSelectedEvent.value = {
      ...modSelectedEvent.value,
...
async function refreshModBadge() {
  try {
    const res = await api.get("/admin/moderation/events", {
      params: { unreviewed: "1", per_page: 1 },
    });
...
async function loadSpendChart() {
  ...
  try {
    const res = await api.get("/admin/spend-chart", {
      params: { days: spendRange.value },
    });
  ...
  try {
    const res = await api.get("/admin/users", {
      params: {
        ...
        status: usersStatus.value || undefined,
        plan: usersPlan.value || undefined,
    ...
    if (Array.isArray(servedTiers) && servedTiers.length)
      planTiers.value = servedTiers;
...
async function impersonate(userId) {
  const ok = await showConfirm(
    "Start an impersonation session as this user? A new tab will open logged in as them.",
    { title: "Impersonate user", confirmLabel: "Impersonate", danger: true }
  );
  if (!ok) return;
  ...
  try {
    ...
    await showAlert(
      e?.response?.data?.error?.message || "Impersonation failed."
    );
  } finally {
...
async function suspendWorkspace(workspaceId, currentStatus) {
  ...
  const ok = await showConfirm(
    `This will ${label} the workspace and affect all users in it.`,
    {
      title: `${label.charAt(0).toUpperCase() + label.slice(1)} workspace?`,
  ...
  try {
    await api.patch(`/admin/workspaces/${workspaceId}/status`, {
      status: newStatus,
    });
    await loadUsers();
    if (panelData.value) {
    ...
    await showAlert(
      e?.response?.data?.error?.message || "Could not update workspace status."
    );
  } finally {
...
const panelSpendMax = computed(() => {
  const vals = (panelData.value?.spend_by_day ?? []).map((d) =>
    Number(d.spend || 0)
  );
  return Math.max(...vals, 0.01);
});
...
async function loadClientWorkspaces(page = 1) {
  ...
  try {
    ...
    clientPagination.value = res.data.meta?.pagination ?? {
      current_page: 1,
      last_page: 1,
      total: 0,
    };
...
async function loadWorkspaces() {
  ...
  try {
    const res = await api.get("/admin/workspaces", {
      params: {
        search: wsSearch.value || undefined,
        status: wsStatus.value || undefined,
        plan: wsPlan.value || undefined,
        page: wsPage.value,
        per_page: wsPerPage.value,
      },
    });
    wsData.value = res.data.data?.workspaces ?? [];
    const served = res.data.meta?.plan_tiers;
    planTiers.value =
      Array.isArray(served) && served.length ? served : PLAN_TIER_FALLBACK;
...
async function updateWsPlan(ws, newPlan) {
  ...
  try {
    ...
    await showAlert(
      e?.response?.data?.error?.message || "Could not update plan."
    );
  } finally {
    wsSaving.value = null;
  }
...
async function toggleWsStatus(ws) {
  ...
  const label = newStatus === "suspended" ? "suspend" : "unsuspend";
  const ok = await showConfirm(
    `This will ${label} "${ws.name}" and affect all its users.`,
    {
      title: `${label.charAt(0).toUpperCase() + label.slice(1)} workspace?`,
      confirmLabel: label.charAt(0).toUpperCase() + label.slice(1),
      danger: newStatus === "suspended",
...
async function loadJobs() {
  ...
  try {
    const res = await api.get("/admin/jobs", {
      params: { page: jobsPage.value, per_page: jobsPerPage.value },
    });
...
async function loadFailures() {
  ...
  try {
    const params = {
      page: failuresPage.value,
      per_page: failuresPerPage.value,
    };
...
function failuresFilter() {
  failuresPage.value = 1;
  loadFailures();
}
function toggleTrace(id) {
  failuresExpandedId.value = failuresExpandedId.value === id ? null : id;
}
...
async function loadAudit() {
  ...
  try {
    const res = await api.get("/admin/audit-log", {
      params: { page: auditPage.value, per_page: auditPerPage.value },
    });
...
function hooksSwitchProvider(p) {
  hooksProvider.value = p;
  hooksPage.value = 1;
  hooksExpanded.value = null;
  loadHooks();
}
function hooksFilter() {
  hooksPage.value = 1;
  loadHooks();
}
...
async function loadBillingChart() {
  ...
  try {
    const res = await api.get("/admin/spend-chart", {
      params: { days: billingRange.value },
    });
...
async function suspendUserWorkspace(user) {
  ...
  const label = newStatus === "suspended" ? "suspend" : "unsuspend";
  const ok = await showConfirm(
    `This will ${label} the workspace for ${user.name || user.email}.`,
    {
      title: `${label.charAt(0).toUpperCase() + label.slice(1)} workspace?`,
      confirmLabel: label.charAt(0).toUpperCase() + label.slice(1),
      danger: newStatus === "suspended",
  ...
  try {
    await api.patch(`/admin/workspaces/${user.workspace_id}/status`, {
      status: newStatus,
    });
    await loadUsers();
  } catch (e) {
...
const projectExamples = [
  {
    label: "Stock video (10 scenes + export)",
    scenes: 10,
    credits: 2 + 1 + 10 * 1 + 10 * 2 + 5,
  },
  {
    label: "Stock images (10 scenes + export)",
    scenes: 10,
    credits: 2 + 1 + 10 * 1 + 10 * 2 + 5,
  },
  {
    label: "Audiogram (10 scenes + export)",
    scenes: 10,
    credits: 2 + 1 + 10 * 1 + 10 * 2 + 5,
  },
  {
    label: "AI images medium (10 scenes + export)",
    scenes: 10,
    credits: 2 + 1 + 10 * 15 + 10 * 2 + 5,
  },
  {
    label: "AI images high (10 scenes + export)",
    scenes: 10,
    credits: 2 + 1 + 10 * 40 + 10 * 2 + 5,
  },
];
...
async function uploadSfx() {
  ...
  try {
    for (let i = 0; i < sfxQueuedFiles.value.length; i++) {
      ...
      const name =
        sfxQueuedFiles.value.length === 1 && sfxUploadForm.value.name
          ? sfxUploadForm.value.name
          : file.name.replace(/\.[^.]+$/, "").slice(0, 60);
      fd.append("name", name);
      if (sfxUploadForm.value.category)
        fd.append("category", sfxUploadForm.value.category);
      if (sfxUploadForm.value.source)
        fd.append("source", sfxUploadForm.value.source);
      fd.append("file", file);
      await api.post("/admin/sfx", fd, {
...
function toggleSfxPreview(sound) {
  ...
  if (sfxPlayingId.value === sound.id) {
    sfxPlayingId.value = null;
    return;
  }
  sfxPreviewAudio = new Audio(sound.preview_url);
  sfxPreviewAudio.addEventListener("ended", () => {
    if (sfxPlayingId.value === sound.id) sfxPlayingId.value = null;
  });
  sfxPreviewAudio.play().catch(() => {
    sfxPlayingId.value = null;
  });
...
function navigate(view) {
  ...
  if (view === "dashboard" && !dashData.value) {
    loadDashboard();
    loadSpendChart();
  }
  ...
  <div class="gm-shell">
    ...
    <aside class="gm-sidebar">
      ...
      <nav class="gm-nav">
        ...
        <button
          :class="['nav-item', activeView === 'users' ? 'active' : '']"
        ...
        <button
          :class="['nav-item', activeView === 'workspaces' ? 'active' : '']"
        ...
        <button
          :class="[
            'nav-item',
            activeView === 'client-workspaces' ? 'active' : '',
          ]"
          @click="navigate('client-workspaces')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M4 4v10a3 3 0 003 3h3M4 4h0m6 13h10m-10 0v-2m0 2v2m10-7v10a1 1 0 01-1 1h-8a1 1 0 01-1-1v-10a1 1 0 011-1h8a1 1 0 011 1z"
            />
          </svg>
          Client Workspaces
          <span v-if="clientPagination.total" class="nav-count">{{
            clientPagination.total
          }}</span>
        </button>
        <div class="nav-section-label">Content</div>
        <button
          :class="['nav-item', activeView === 'videos' ? 'active' : '']"
        ...
        <button
          :class="['nav-item', activeView === 'plans' ? 'active' : '']"
        ...
        <button
          :class="['nav-item', activeView === 'sfx' ? 'active' : '']"
          @click="navigate('sfx')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
        ...
        <button
          :class="['nav-item', activeView === 'affiliates' ? 'active' : '']"
          @click="navigate('affiliates')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              stroke-linecap="round"
        ...
        <button
          :class="['nav-item', activeView === 'jobs' ? 'active' : '']"
          @click="navigate('jobs')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
          ...
          Jobs
          <span
            v-if="(jobsData?.counts?.failed_today ?? 0) > 0"
            class="nav-badge"
            >{{ jobsData.counts.failed_today }}</span
          >
        </button>
        <button
          :class="['nav-item', activeView === 'billing' ? 'active' : '']"
        ...
        <button
          :class="['nav-item', activeView === 'hooks' ? 'active' : '']"
          @click="navigate('hooks')"
        >
          <span class="nav-icon">⇄</span><span class="nav-label">Webhooks</span>
        </button>
        <button
          :class="['nav-item', activeView === 'audit' ? 'active' : '']"
        ...
        <button
          :class="['nav-item', activeView === 'failures' ? 'active' : '']"
          @click="navigate('failures')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              stroke-linecap="round"
        ...
        <button
          :class="['nav-item', activeView === 'moderation' ? 'active' : '']"
          @click="navigate('moderation')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              stroke-linecap="round"
          ...
          Trust &amp; Safety
          <span v-if="modUnreviewedCount > 0" class="nav-badge">{{
            modUnreviewedCount
          }}</span>
        </button>
        <button
          :class="['nav-item', activeView === 'mail' ? 'active' : '']"
          @click="navigate('mail')"
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
      ...
      <div class="gm-sidebar-foot">
        <div class="gm-user-row">
          <div class="gm-avatar" style="background: #7c3aed33; color: #7c3aed">
            {{ initials(authStore.user?.name, authStore.user?.email) }}
          </div>
          <div>
            <div class="gm-user-name">
    ...
    <main class="gm-main">
      ...
      <div class="gm-content">
            ...
            <div class="metrics-grid">
              <div class="metric-card">
                ...
                <div class="metric-value">
                  {{ dashData.summary?.total_users ?? 0 }}
                </div>
                <div class="metric-sub">
                  {{ dashData.summary?.active_users ?? 0 }} active
                </div>
              </div>
              <div class="metric-card blue">
                <div class="metric-label">Workspaces</div>
                <div class="metric-value">
                  {{ dashData.summary?.total_workspaces ?? 0 }}
                </div>
                <div class="metric-sub">
                  {{ dashData.summary?.total_projects ?? 0 }} total projects
                </div>
              </div>
              <div class="metric-card green">
                <div class="metric-label">Exports Today</div>
                <div class="metric-value">
                  {{ dashData.summary?.exports_today ?? 0 }}
                </div>
                <div class="metric-sub">
                  {{ dashData.summary?.exports_month ?? 0 }} this month
                </div>
              </div>
              <div class="metric-card purple">
                <div class="metric-label">AI Spend (Month)</div>
                <div class="metric-value">
                  {{ money(dashData.summary?.api_spend_month_usd) }}
                </div>
                <div class="metric-sub">
                  {{ money(dashData.summary?.api_spend_today_usd) }} today
                </div>
              </div>
              <div class="metric-card yellow">
                <div class="metric-label">Total Tracked</div>
                <div class="metric-value">
                  {{ money(dashData.summary?.api_spend_total_usd) }}
                </div>
              ...
              <div class="metric-card red">
                ...
                <div class="metric-value">
                  {{ dashData.summary?.failed_jobs_today ?? 0 }}
                </div>
                    ...
                    <div class="chart-bars">
                      ...
                      >
                        <div
                          ...
                          :style="{
                            height: `${Math.round(
                              (Number(day.spend) / spendChartMax) * 100
                            )}%`,
                          }"
                        ></div>
                      </div>
                      <div
                        v-if="spendChart.length === 0"
                        class="gm-spinner-wrap"
                        style="height: 80px"
                      >
                        No data yet.
                  ...
                  >
                    ...
                    >
                      <span
                        ...
                        >{{ item.plan }}</span
                      >
                      <span style="font-size: 12px; color: var(--gm-muted)"
                    ...
                    <div class="spend-bar">
                      <div
                        class="spend-fill"
                        :style="{ width: `${Math.min(100, item.pct * 3)}%` }"
                      ></div>
                    </div>
                  </div>
                  <div
                    v-if="!dashData.plan_distribution?.length"
                    style="color: var(--gm-muted); font-size: 13px"
                  >
                    No workspace data.
                  </div>
                </div>
                ...
                <table>
                  ...
                  <tbody>
                    <tr v-for="ws in dashData.top_spenders ?? []" :key="ws.id">
                      ...
                      <td>
                        <span
                          :class="['badge', planBadgeClass(ws.plan_tier)]"
                          >{{ ws.plan_tier }}</span
                        >
                      </td>
                      <td class="cell-muted">{{ ws.owner_email }}</td>
                      <td class="cell-spend">
                        {{ money(ws.spend_month_usd) }}
                      </td>
                    </tr>
        ...
        <template v-if="activeView === 'users'">
          <div class="section">
            <div class="section-header">
              ...
              <div class="section-actions">
                <span class="meta-count"
                  >{{ usersPagination.total ?? 0 }} users</span
                >
              </div>
            </div>
            <div class="filters">
              ...
              >
                ...
                <option v-for="t in planTiers" :key="t" :value="t">
                  {{ planLabel(t) }}
                </option>
              </select>
            </div>
              ...
              <table v-else>
                ...
                <tbody>
                  <tr v-for="u in usersData" :key="u.id">
                    ...
                    <td>
                      <span :class="['badge', planBadgeClass(u.plan_tier)]">{{
                        u.plan_tier || "-"
                      }}</span>
                    </td>
                    <td>
                      ...
                      <div
                        v-if="u.workspace_status"
                        :class="[
                          'cell-sub',
                          u.workspace_status !== 'active' ? 'text-red' : '',
                        ]"
                      >
                        {{ u.workspace_status }}
                      </div>
                    ...
                    <td>
                      <div style="display: flex; gap: 5px; flex-wrap: wrap">
                        <button
                          class="btn btn-ghost btn-sm"
                          @click="openPanel(u.id)"
                        >
                          View
                        </button>
                        <button
                          class="btn btn-impersonate btn-sm"
                          title="Impersonate"
            ...
            <div class="gm-pagination">
              ...
              <span class="pg-info"
                >{{ usersPagination.from ?? 0 }}–{{
                  usersPagination.to ?? 0
                }}
                of {{ usersPagination.total ?? 0 }}</span
              >
              <div class="pg-controls">
                ...
                <span
                  >{{ usersPage }} / {{ usersPagination.last_page ?? 1 }}</span
                >
                <button
                  :disabled="usersPage >= (usersPagination.last_page ?? 1)"
                  @click="
                    usersPage++;
                    loadUsers();
                  "
        ...
        <template v-if="activeView === 'client-workspaces'">
          <div class="section">
            <div class="section-header">
              ...
              <div class="section-actions">
                <span class="meta-count"
                  >{{ clientPagination.total ?? 0 }} across all agencies</span
                >
              </div>
            </div>
            <div class="filters">
            ...
            <div class="table-wrap">
              <table class="gm-table">
                ...
                <tbody>
                  ...
                  <tr v-else-if="!clientData.length">
                  ...
                  <template v-for="c in clientData" v-else :key="c.id">
                    <tr
                      :class="[
                        'cw-parent',
                        clientExpanded.has(c.id) ? 'is-open' : '',
                      ]"
                    ...
                    >
                      ...
                      <td>
                        {{
                          c.monthly_credit_cap
                            ? c.monthly_credit_cap.toLocaleString()
                            : "—"
                        }}
                      </td>
            ...
            <div v-if="clientPagination.last_page > 1" class="pagination">
              <button
                class="btn btn-ghost btn-sm"
                :disabled="clientPagination.current_page <= 1"
                @click="loadClientWorkspaces(clientPagination.current_page - 1)"
              >
                Previous
              </button>
              <span class="meta-count"
                >Page {{ clientPagination.current_page }} of
                {{ clientPagination.last_page }}</span
              >
              <button
                class="btn btn-ghost btn-sm"
                :disabled="
        ...
        <template v-if="activeView === 'workspaces'">
          <div class="section">
                  ...
                  >
                    ...
                    <td>
                      <select
                        ...
                        @change="updateWsPlan(ws, $event.target.value)"
                      >
                        <option
                          v-if="
                            ws.plan_tier && !planTiers.includes(ws.plan_tier)
                          "
                          :value="ws.plan_tier"
                        >
                    ...
                    <td>
                      <span
                        :class="[
                          'badge',
                          ws.status === 'active' ? 'badge-green' : 'badge-red',
                        ]"
                        >{{ ws.status }}</span
                      >
                    </td>
                    ...
                    <td>
                      <button
                        :class="[
                          'btn btn-sm',
                          ws.status === 'active' ? 'btn-danger' : 'btn-ghost',
                        ]"
                        :disabled="wsSaving === ws.id"
                        @click="toggleWsStatus(ws)"
                      >
                        {{ ws.status === "active" ? "Suspend" : "Unsuspend" }}
                      </button>
                    </td>
                  </tr>
            ...
            <div class="gm-pagination">
              ...
              <span class="pg-info"
                >{{ wsPagination.from ?? 0 }}–{{ wsPagination.to ?? 0 }} of
                {{ wsPagination.total ?? 0 }}</span
              >
              <div class="pg-controls">
                <button
                  :disabled="wsPage <= 1"
                ...
                <button
                  :disabled="wsPage >= (wsPagination.last_page ?? 1)"
                  @click="
                    wsPage++;
                    loadWorkspaces();
                  "
          ...
          <template v-else-if="jobsData">
            ...
            <div class="metrics-grid metrics-grid-4">
              <div class="metric-card blue">
                ...
                <div class="metric-value">
                  {{ jobsData.counts?.processing ?? 0 }}
                </div>
              </div>
              <div class="metric-card yellow">
                <div class="metric-label">Queued</div>
                <div class="metric-value">
                  {{ jobsData.counts?.queued ?? 0 }}
                </div>
              </div>
              <div class="metric-card green">
                <div class="metric-label">Completed Today</div>
                <div class="metric-value">
                  {{ jobsData.counts?.completed_today ?? 0 }}
                </div>
              </div>
              <div class="metric-card red">
                <div class="metric-label">Failed Today</div>
                <div class="metric-value">
                  {{ jobsData.counts?.failed_today ?? 0 }}
                </div>
              </div>
            </div>
            <div class="section">
              <div class="section-header">
                ...
                <div class="section-actions">
                  <button class="btn btn-ghost btn-sm" @click="loadJobs()">
                    Refresh
                  </button>
                </div>
              </div>
                  ...
                  <tbody>
                    <tr v-for="job in jobsData.jobs ?? []" :key="job.id">
                      <td>
                        ...
                        <div
                          v-if="job.failure_reason"
                          class="cell-sub cell-error"
                      ...
                      <td class="cell-muted">
                        {{ job.workspace_name || `WS #${job.workspace_id}` }}
                      </td>
                      <td>
                        <span
                          :class="[
                      ...
                      <td class="cell-muted">
                        {{
                          job.render_seconds != null
                            ? `${Math.round(job.render_seconds)}s`
                            : "-"
                        }}
                      </td>
                      <td class="cell-muted">
                        {{
                          job.render_cost_usd != null
                            ? `$${Number(job.render_cost_usd).toFixed(4)}`
                            : "-"
                        }}
                      </td>
              ...
              <div class="gm-pagination">
                ...
                <span class="pg-info"
                  >{{ jobsPagination.from ?? 0 }}–{{
                    jobsPagination.to ?? 0
                  }}
                  of {{ jobsPagination.total ?? 0 }}</span
                >
                <div class="pg-controls">
                  <button
                    :disabled="jobsPage <= 1"
                  ...
                  <span
                    >{{ jobsPage }} / {{ jobsPagination.last_page ?? 1 }}</span
                  >
                  <button
                    :disabled="jobsPage >= (jobsPagination.last_page ?? 1)"
                    @click="
                      jobsPage++;
                      loadJobs();
                    "
        ...
        <template v-if="activeView === 'audit'">
          <div class="section">
            ...
            <div class="gm-pagination">
              ...
              <span class="pg-info"
                >{{ auditPagination.from ?? 0 }}–{{
                  auditPagination.to ?? 0
                }}
                of {{ auditPagination.total ?? 0 }}</span
              >
              <div class="pg-controls">
                <button
                ...
                <span
                  >{{ auditPage }} / {{ auditPagination.last_page ?? 1 }}</span
                >
                <button
                  :disabled="auditPage >= (auditPagination.last_page ?? 1)"
                  @click="
                    auditPage++;
                    loadAudit();
                  "
        ...
        <template v-if="activeView === 'hooks'">
          <div class="section">
              ...
              <div class="section-actions">
                <button
                  :class="[
                    'btn',
                    'btn-sm',
                    hooksProvider === 'kelviq' ? 'btn-primary' : '',
                  ]"
                  @click="hooksSwitchProvider('kelviq')"
                >
                  Kelviq
                </button>
                <button
                  :class="[
                    'btn',
                    'btn-sm',
                    hooksProvider === 'appsumo' ? 'btn-primary' : '',
                  ]"
                  @click="hooksSwitchProvider('appsumo')"
                >
                  AppSumo
                </button>
                <select
                  v-model="hooksOutcome"
                  class="pg-per-page"
                  style="width: 170px"
                  @change="hooksFilter"
                >
            ...
            <div class="hooks-counts">
              <span
                v-for="(n, key) in hooksCounts"
                :key="key"
                :class="['badge', hookOutcomeClass(key)]"
                >{{ key }}: {{ n }}</span
              >
              <span v-if="!Object.keys(hooksCounts).length" class="cell-muted"
                >No deliveries recorded yet.</span
              >
            </div>
                ...
                <tbody>
                  <template v-for="row in hooksData" :key="row.id">
                    <tr>
                      ...
                      <td>
                        <span
                          :class="[
                            'badge',
                            row.signature_valid ? 'badge-green' : 'badge-red',
                          ]"
                          >{{ row.signature_valid ? "ok" : "bad" }}</span
                        >
                      </td>
                      <td>
                        <span
                          :class="['badge', hookOutcomeClass(row.outcome)]"
                          >{{ row.outcome }}</span
                        >
                      </td>
                      <td class="cell-muted">
                      ...
                      <td>
                        <button
                          ...
                          @click="
                            hooksExpanded =
                              hooksExpanded === row.id ? null : row.id
                          "
                        >
                    ...
                    <tr v-if="hooksExpanded === row.id">
                      <td colspan="7">
                        <div v-if="row.message" class="hook-msg">
                          {{ row.message }}
                        </div>
                        <pre class="hook-payload">{{
                  ...
                  <tr v-if="!hooksLoading && hooksData.length === 0">
                    <td colspan="7" class="empty-cell">
                      No {{ hooksProvider }} webhooks recorded yet.
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="gm-pagination">
              ...
              <span class="pg-info"
                >{{ hooksPagination.total ?? 0 }} total</span
              >
              <div class="pg-controls">
                <button
                  :disabled="hooksPage <= 1"
                  @click="
                    hooksPage--;
                    loadHooks();
                  "
                ...
                <span
                  >{{ hooksPage }} / {{ hooksPagination.last_page ?? 1 }}</span
                >
                <button
                  :disabled="hooksPage >= (hooksPagination.last_page ?? 1)"
                  @click="
                    hooksPage++;
                    loadHooks();
                  "
        ...
        <template v-if="activeView === 'failures'">
          <div class="section">
            <div class="section-header">
              ...
              <div class="section-actions">
                <select
                  v-model="failuresEntityType"
                  class="pg-per-page"
                  style="width: 140px"
                  @change="failuresFilter"
                >
            ...
            <div class="table-wrap">
              <div v-if="failuresLoading" class="gm-spinner-wrap">
                Loading...
              </div>
              <table v-else>
                <thead>
                ...
                <tbody v-for="t in failuresData" :key="t.id">
                  <tr>
                    ...
                    <td>
                      <span class="badge badge-red">{{ t.job_label }}</span>
                    </td>
                    <td class="cell-muted">
                      <span v-if="t.entity_type"
                        >{{ t.entity_type }} #{{ t.entity_id }}</span
                      >
                      <span v-else>—</span>
                    </td>
                    <td
            ...
            <div class="gm-pagination">
              <span class="pg-label">Rows per page</span>
              <select
                v-model="failuresPerPage"
                class="pg-per-page"
                @change="failuresFilter"
              >
                <option v-for="n in [20, 50, 100]" :key="n" :value="n">
                  {{ n }}
                </option>
              </select>
              <span class="pg-info"
                >{{ failuresPagination.current_page ?? 1 }} /
                {{ failuresPagination.last_page ?? 1 }} ·
                {{ failuresPagination.total ?? 0 }} total</span
              >
              <div class="pg-controls">
                <button
                ...
                <button
                  :disabled="
                    failuresPage >= (failuresPagination.last_page ?? 1)
                  "
                  @click="
                    failuresPage++;
                    loadFailures();
        ...
        <template v-if="activeView === 'videos'">
          ...
          <div class="status-chips">
            <button
              ...
              :class="[
                'status-chip',
                videosStatus === chip.value ? 'active' : '',
              ]"
              @click="
                videosStatus = chip.value;
                videosFilter();
              "
            >
              {{ chip.label }}
              <span class="chip-count">{{
                chip.value === ""
                  ? videosStatusCounts.all ?? 0
                  : videosStatusCounts[chip.value] ?? 0
              }}</span>
            </button>
          </div>
          ...
          <div class="section">
                ...
                <tbody>
                  <tr v-for="v in videosData" :key="v.id">
                    <td>
                      ...
                      <div class="cell-sub">
                        {{ v.source_type
                          ...
                          v.primary_language ? ` · ${v.primary_language}` : ""
                        }}
                      </div>
                    </td>
                    <td>
                      <span :class="['badge', videoStatusBadge(v.status)]">{{
                    ...
                    <td>
                      ...
                      <div class="cell-sub">
                        <span
                          :class="['badge', planBadgeClass(v.plan_tier)]"
            ...
            <div class="gm-pagination">
              ...
              <span class="pg-info"
                >{{ videosPagination.from ?? 0 }}–{{
                  videosPagination.to ?? 0
                }}
                of {{ videosPagination.total ?? 0 }}</span
              >
              <div class="pg-controls">
                <button
                ...
                <span
                  >{{ videosPage }} /
                  {{ videosPagination.last_page ?? 1 }}</span
                >
                <button
                  :disabled="videosPage >= (videosPagination.last_page ?? 1)"
                  @click="
                    videosPage++;
                    loadVideos();
                  "
        ...
        <template v-if="activeView === 'billing'">
          ...
          <div v-if="dashData?.summary" class="metrics-grid metrics-grid-4">
            <div class="metric-card purple">
              ...
              <div class="metric-value">
                {{ money(dashData.summary.api_spend_today_usd) }}
              </div>
            </div>
            <div class="metric-card blue">
              ...
              <div class="section-actions">
                <div class="spend-toggle">
                  <button
                    ...
                    :class="[
                      'spend-toggle-btn',
                      billingRange === d ? 'active' : '',
                    ]"
                    @click="billingRange = d"
                  >
                    {{ d }}d
                  </button>
                </div>
                ...
                <div class="chart-bars" style="height: 140px">
                  ...
                  >
                    <div
                      ...
                      :style="{
                        height: `${Math.round(
                          (Number(day.spend) / billingChartMax) * 100
                        )}%`,
                      }"
                    ></div>
                  </div>
                  <div
                    v-if="billingChart.length === 0"
                    class="gm-spinner-wrap"
                    style="flex: 1"
                  >
                    No data yet.
          ...
          <div class="section">
            <div class="section-header">
              <div class="section-title">Top Spending Workspaces</div>
            </div>
            <div class="table-wrap">
              <table>
                <thead>
                ...
                <tbody>
                  <tr v-for="ws in dashData?.top_spenders ?? []" :key="ws.id">
                    ...
                    <td>
                      <span :class="['badge', planBadgeClass(ws.plan_tier)]">{{
                        ws.plan_tier
                      }}</span>
                    </td>
                    <td class="cell-muted">{{ ws.owner_email }}</td>
        ...
        <template v-if="activeView === 'storage'">
          <div v-if="storageLoading && !storageData" class="gm-spinner-wrap">
          ...
          <template v-else>
            ...
            <div class="metrics-grid metrics-grid-4">
              <div class="metric-card">
                ...
                <div class="metric-value">
                  {{ storageData?.summary?.total_human ?? "—" }}
                </div>
              ...
              <div class="metric-card blue">
                ...
                <div class="metric-value">
                  {{
                    (storageData?.summary?.total_assets ?? 0).toLocaleString()
                  }}
                </div>
                <div class="metric-sub">
                  {{ storageData?.summary?.workspace_count ?? 0 }} workspaces
                </div>
              </div>
              <div class="metric-card green">
                ...
                <div class="metric-value">
                  {{
                    (storageData?.summary?.video_count ?? 0).toLocaleString()
                  }}
                </div>
                <div class="metric-sub">
                  {{
                    (storageData?.summary?.audio_count ?? 0).toLocaleString()
                  }}
                  audio files
                </div>
              </div>
              <div class="metric-card purple">
                ...
                <div class="metric-value">
                  {{
                    (storageData?.summary?.image_count ?? 0).toLocaleString()
                  }}
                </div>
            ...
            <div class="section">
              <div class="section-header">
                ...
                <div class="section-actions">
                  <span class="meta-count"
                    >{{ storagePagination.total ?? 0 }} workspaces</span
                  >
                  <button
                    class="btn btn-ghost btn-sm"
              ...
              <div class="table-wrap">
                <div v-if="storageLoading" class="gm-spinner-wrap">
                  Loading...
                </div>
                <table v-else>
                  <thead>
                  ...
                  <tbody>
                    ...
                    <tr
                      v-for="row in storageData?.by_workspace"
                      :key="row.workspace_id"
                    >
                      <td class="cell-name">{{ row.workspace_name }}</td>
                      <td>
                        <span
                          :class="['badge', planBadgeClass(row.plan_tier)]"
                          >{{ row.plan_tier }}</span
                        >
                      </td>
                      ...
                      <td class="cell-muted">
                        {{ row.video_count.toLocaleString() }}
                      </td>
                      <td class="cell-muted">
                        {{ row.audio_count.toLocaleString() }}
                      </td>
                      <td class="cell-muted">
                      ...
                      <td>
                        <div class="storage-bar-wrap">
                          <div
                            ...
                            :style="{
                              width: storageBarWidth(row.total_bytes) + '%',
                            }"
                          ></div>
                        </div>
                      </td>
              ...
              <div class="gm-pagination">
                <span class="pg-label">Rows per page</span>
                <select
                  v-model="storagePerPage"
                  class="pg-per-page"
                  @change="storagePerPageChange"
                >
                  <option v-for="n in PER_PAGE_OPTIONS" :key="n" :value="n">
                    {{ n }}
                  </option>
                ...
                <span class="pg-info"
                  >{{ storagePagination.from ?? 0 }}–{{
                    storagePagination.to ?? 0
                  }}
                  of {{ storagePagination.total ?? 0 }}</span
                >
                <div class="pg-controls">
                  <button
                  ...
                  <span
                    >{{ storagePage }} /
                    {{ storagePagination.last_page ?? 1 }}</span
                  >
                  <button
                    :disabled="
                      storagePage >= (storagePagination.last_page ?? 1)
                    "
                    @click="
                      storagePage++;
                      loadStorage();
          ...
          <div class="mail-page">
            <div class="mail-compose">
              ...
              <p
                ...
                "
              >
                Sends from <strong>hello@wyvstudio.com</strong>. Broadcasts skip
                internal accounts and suspended (refunded) workspaces
                automatically, and every send is written to the audit log.
              </p>
              ...
              <div class="mail-form">
                ...
                <template v-if="mailSegment === 'custom'">
                  ...
                  >
                    <span
                      >{{ mailCustomList.length }} recipient{{
                        mailCustomList.length === 1 ? "" : "s"
                      }}</span
                    >
                    <button
                      v-if="mailCustomList.length === 1"
                      class="btn btn-ghost btn-sm"
                      type="button"
                      :disabled="mailDrafting"
                      @click="draftFromUsage"
                    >
                      {{
                        mailDrafting
                          ? "Reading their usage…"
                          : "✨ Draft from their usage"
                      }}
                    </button>
                  </div>
                  <div v-if="mailDossier" class="mail-dossier">
                    <div style="font-weight: 600; margin-bottom: 4px">
                      What the draft is based on (facts from our DB):
                    </div>
                    <pre>{{ mailDossier }}</pre>
                  </div>
                </template>
                <div v-else-if="mailRecipients" class="mail-hint">
                  ...
                  >
                    <span>
                      <strong>{{ mailRecipientCount }}</strong> of
                      {{ mailRecipients.count }} recipient{{
                        mailRecipients.count === 1 ? "" : "s"
                      }}
                      <template v-if="mailExcluded.size">
                        · {{ mailExcluded.size }} removed</template
                      >
                    </span>
                    <button
                      class="btn btn-ghost btn-sm"
                      type="button"
                      @click="mailListOpen = !mailListOpen"
                    >
                  ...
                  <div v-if="!mailListOpen" style="margin-top: 4px">
                    {{ mailRecipients.sample.join(", ")
                    }}{{
                      mailRecipients.count > mailRecipients.sample.length
                        ? "…"
                        : ""
                    }}
                  </div>
                  ...
                  <div v-else class="mail-reciplist">
                    ...
                    <label
                      ...
                      :class="[
                        'mail-recip',
                        mailExcluded.has(r.email) ? 'is-out' : '',
                      ]"
                    >
                      <input
                        type="checkbox"
                      ...
                      <span v-if="r.name" class="mail-recip-name">{{
                        r.name
                      }}</span>
                    </label>
                  </div>
                ...
                <div class="mail-draft-row">
                  <button
                    ...
                    @click="draftMail"
                  >
                    {{ mailDrafting ? "Drafting…" : "Draft it" }}
                  </button>
                  <span class="mail-hint">
                    <template v-if="mailDraftAudience">
                      Written from what we know about
                      {{ mailDraftAudience.label }}. Edit anything below.
                    </template>
                    <template v-else>
                ...
                <textarea
                  ...
                  class="mail-input mail-textarea"
                  rows="12"
                  placeholder="Plain text. Blank line = new paragraph. Use {name} to insert the customer's first name."
                ></textarea>
                <div class="mail-hint">
                  Plain text. Any <code v-pre>https://</code> link becomes
                  clickable; a blank line starts a new paragraph.
                  <code v-pre>{name}</code> becomes each recipient's first name
                  ("there" if unknown).
                </div>

                <div v-if="mailError" class="mail-error">{{ mailError }}</div>
                <div v-if="mailResult" class="mail-success">
                  Queued to {{ mailResult.queued }} recipient{{
                    mailResult.queued === 1 ? "" : "s"
                  }}
                  ✓
                </div>
                ...
                <div class="mail-actions">
                  <template v-if="mailConfirming">
                    <span class="mail-confirm-copy"
                      >Send to {{ mailRecipientCount }} people?</span
                    >
                    <button
            ...
            <div class="gm-section-title" style="margin-top: 34px">
              Delivery
            </div>
            <div class="mail-log-bar">
              ...
              />
              <select
                v-model="mailLogStatus"
                class="filter-select"
                @change="filterMailLog"
              >
              ...
              >
                ...
                <option v-for="m in mailLogMailables" :key="m" :value="m">
                  {{ m }}
                </option>
              </select>
              <select
                v-model="mailLogOpenedOnly"
                class="filter-select"
                @change="filterMailLog"
              >
              ...
              <span class="mail-log-counts">
                <span
                  v-for="(n, k) in mailLogCounts"
                  :key="k"
                  :class="[
                    'aff-pill',
                    {
                      delivered: 'aff-pill-ok',
                      opened: 'aff-pill-ok',
                      sent: 'aff-pill-muted',
                      bounced: 'aff-pill-bad',
                      failed: 'aff-pill-bad',
                      complained: 'aff-pill-bad',
                    }[k] ?? 'aff-pill-muted',
                  ]"
                  >{{ k }} {{ n }}</span
                >
            ...
            >
              Mail is being delivered but no opens are recorded — open tracking
              is switched on per sending domain in Resend, and is off until you
              enable it there.
            </div>
            ...
            <table v-else class="gm-table">
              ...
              <tbody>
                <tr v-for="m in mailLog" :key="m.id">
                  ...
                  <td>
                    <div>{{ m.subject || "—" }}</div>
                    <div v-if="m.mailable" class="aff-sub">
                      {{ m.mailable }}
                    </div>
                  </td>
                  <td>
                    <span
                      :class="[
                        'aff-pill',
                        {
                          ...
                          failed: 'aff-pill-bad',
                          complained: 'aff-pill-bad',
                        }[m.status] ?? 'aff-pill-muted',
                      ]"
                      >{{ m.status }}</span
                    >
                    <div v-if="m.failure_reason" class="aff-sub">
                      {{ m.failure_reason }}
                    </div>
                  </td>
                  <td class="cell-muted">
                    <template v-if="m.first_opened_at">
                      ...
                      <span v-if="m.open_count > 1" class="aff-sub"
                        >{{ m.open_count }}×</span
                      >
                    </template>
            ...
            <div v-if="mailLog.length" class="mail-log-bar">
              ...
              <span class="pg-info">
                {{ mailLogPagination.from ?? 0 }}–{{
                  mailLogPagination.to ?? 0
                }}
                of {{ mailLogPagination.total ?? 0 }}
              </span>
              <div class="pg-controls">
                <button
                  :disabled="mailLogPage <= 1"
                  @click="
                    mailLogPage--;
                    loadMailLog();
                  "
                ...
                <span
                  >{{ mailLogPage }} /
                  {{ mailLogPagination.last_page ?? 1 }}</span
                >
                <button
                  :disabled="mailLogPage >= (mailLogPagination.last_page ?? 1)"
                  @click="
                    mailLogPage++;
                    loadMailLog();
                  "
            ...
            <div v-if="!mailHistory.length" class="gm-empty">
              Nothing sent yet.
            </div>
            <table v-else class="gm-table">
              <thead>
              ...
              <tbody>
                <template v-for="m in mailHistory" :key="m.id">
                  <tr
                    class="gm-row-click"
                    @click="
                      mailHistoryOpen = mailHistoryOpen === m.id ? null : m.id
                    "
                  >
                    <td class="cell-muted" style="white-space: nowrap">
                      {{ fmtDate(m.sent_at) }}
                    ...
                    <td class="cell-muted" style="white-space: nowrap">
                      <template v-if="m.stats">
                        <strong>{{ m.stats.opened }}</strong> /
                        {{ m.stats.delivered }}
                        <span
                          v-if="m.stats.bounced"
                          class="aff-sub"
                          style="color: #fca5a5"
                        >
                  ...
                  <tr v-if="mailHistoryOpen === m.id">
                    <td colspan="5" class="mail-history-detail">
                      ...
                      <div v-else class="mail-hint">
                        Message text wasn't stored for this send (sent before
                        body logging).
                      </div>
                    </td>
                  </tr>
            ...
            <div v-if="mailHistory.length" class="mail-log-bar">
              <select
                v-model.number="mailHistoryPerPage"
                class="filter-select"
                @change="
                  mailHistoryPage = 1;
                  loadMailHistory();
              ...
              <span class="pg-info">
                {{ mailHistoryPagination.from ?? 0 }}–{{
                  mailHistoryPagination.to ?? 0
                }}
                of {{ mailHistoryPagination.total ?? 0 }}
              </span>
              <div class="pg-controls">
                <button
                  :disabled="mailHistoryPage <= 1"
                  @click="
                    mailHistoryPage--;
                    loadMailHistory();
                  "
                ...
                <span
                  >{{ mailHistoryPage }} /
                  {{ mailHistoryPagination.last_page ?? 1 }}</span
                >
                <button
                  :disabled="
                    mailHistoryPage >= (mailHistoryPagination.last_page ?? 1)
                  "
                  @click="
                    mailHistoryPage++;
                    loadMailHistory();
        ...
        <template v-if="activeView === 'moderation'">
          <div class="moderation-page">
            ...
            <p
              ...
              "
            >
              Provider rejections, user reports, and pattern alerts. Click any
              row to triage.
            </p>
            ...
            <div
              v-if="modLoading"
              style="padding: 24px; color: var(--gm-muted); font-size: 13px"
            >
              Loading…
            </div>
            <table v-else class="gm-table">
              ...
              <tbody>
                <tr v-if="modEvents.length === 0">
                  <td
                    ...
                    "
                  >
                    No events match the current filter. Quiet days are good
                    news.
                  </td>
                </tr>
                <tr
                  v-for="e in modEvents"
                  :key="e.id"
                  class="gm-row-click"
                  @click="openModEvent(e.id)"
                >
                  <td class="cell-muted">{{ fmtDate(e.created_at) }}</td>
                  <td>{{ modSourceLabel(e.source) }}</td>
                  <td>
                    <span :class="['mod-sev', modSeverityClass(e.severity)]">{{
                      e.severity
                    }}</span>
                  </td>
                  <td>
                    <div style="font-weight: 500">
                      {{ e.workspace?.name || "—" }} #{{ e.workspace?.id }}
                    </div>
                  ...
                  <td>
                    <span v-if="e.reviewed_at" class="mod-pill mod-pill-done"
                      >Reviewed</span
                    >
          ...
          <!-- Event detail modal -->
          <div
            v-if="modSelectedEvent"
            class="gm-modal-overlay"
            @click.self="modSelectedEvent = null"
          >
            <div class="gm-modal mod-detail">
              <div class="gm-modal-head">
                <div>
                  <div class="gm-modal-title">
                    Event #{{ modSelectedEvent.event.id }}
                  </div>
                  <div
                    style="
                      font-size: 12px;
                    ...
                    "
                  >
                    {{ modSourceLabel(modSelectedEvent.event.source) }} ·
                    {{ fmtDate(modSelectedEvent.event.created_at) }}
                  </div>
                </div>
                <button class="gm-modal-close" @click="modSelectedEvent = null">
                  ✕
                </button>
              </div>
              <div class="gm-modal-body">
                <div class="mod-detail-row">
                  ...
                  ><span
                    :class="[
                      'mod-sev',
                      modSeverityClass(modSelectedEvent.event.severity),
                    ]"
                    >{{ modSelectedEvent.event.severity }}</span
                  >
                </div>
                <div class="mod-detail-row">
                  <span>Workspace</span
                  ><span
                    >{{ modSelectedEvent.event.workspace?.name || "—" }} #{{
                      modSelectedEvent.event.workspace?.id
                    }}</span
                  >
                </div>
                <div class="mod-detail-row">
                  <span>User</span
                  ><span>{{ modSelectedEvent.event.user?.email || "—" }}</span>
                </div>
                <div class="mod-detail-row">
                  <span>Operation</span
                  ><span>{{ modSelectedEvent.event.operation || "—" }}</span>
                </div>
                <div
                  v-if="modSelectedEvent.event.project_id"
                  class="mod-detail-row"
                >
                  <span>Project</span
                  ><span
                    >#{{ modSelectedEvent.event.project_id }} · scene #{{
                      modSelectedEvent.event.scene_id || "—"
                    }}</span
                  >
                </div>
                <div class="mod-detail-row">
                ...
                <!-- Pattern-alert specifics: WHAT matched and WHERE, instead of
                     an opaque one-liner. -->
                <div
                  v-if="modSelectedEvent.event.metadata?.term"
                  class="mod-detail-row"
                >
                  <span>Matched term</span>
                  <span
                    ><code class="mod-term">{{
                      modSelectedEvent.event.metadata.term
                    }}</code>
                    <span
                      v-if="modSelectedEvent.event.metadata.tier"
                      class="mod-tier"
                      >{{ modSelectedEvent.event.metadata.tier }}</span
                    ></span
                  >
                </div>
                <div
                  v-if="modSelectedEvent.event.metadata?.match_context"
                  class="mod-detail-row"
                >
                  ...
                  <span
                    ...
                    v-html="
                      modHighlight(
                        modSelectedEvent.event.metadata.match_context,
                        modSelectedEvent.event.metadata.term
                      )
                    "
                  ></span>
                ...
                <div
                  v-if="modSelectedEvent.event.prompt"
                  class="mod-detail-row"
                >
                ...
                <div
                  v-if="modSelectedEvent.event.report_url"
                  class="mod-detail-row"
                >
                  <span>Reported URL</span
                  ><span
                    ><a
                      :href="modSelectedEvent.event.report_url"
                      target="_blank"
                      rel="noopener"
                      >{{ modSelectedEvent.event.report_url }}</a
                    ></span
                  >
                </div>
                <div
                  v-if="modSelectedEvent.event.report_message"
                  class="mod-detail-row"
                >
                  <span>Reporter message</span
                  ><span style="white-space: pre-wrap">{{
                    modSelectedEvent.event.report_message
                  }}</span>
                </div>
                <div
                  v-if="modSelectedEvent.event.report_email"
                  class="mod-detail-row"
                >
                  <span>Reporter email</span
                  ><span>{{ modSelectedEvent.event.report_email }}</span>
                </div>

                <h4
                  style="margin: 24px 0 10px; font-size: 13px; font-weight: 600"
                >
                  Review &amp; Action
                </h4>
                <div
                  v-if="modSelectedEvent.event.reviewed_at"
                  style="
                    font-size: 12px;
                    color: var(--gm-muted);
                    margin-bottom: 10px;
                  "
                >
                  Reviewed {{ fmtDate(modSelectedEvent.event.reviewed_at) }} by
                  {{ modSelectedEvent.event.reviewer?.email || "—" }} — action:
                  <strong>{{ modSelectedEvent.event.action_taken }}</strong>
                </div>
                <select
                  v-model="modActionDraft.action_taken"
                  class="mod-select"
                ...
                >
                  ...
                  <option value="workspace_terminated">
                    Workspace terminated
                  </option>
                  <option value="reported_to_authorities">
                    Reported to authorities
                  </option>
                </select>
                <textarea
                  v-model="modActionDraft.action_notes"
                  placeholder="Notes (optional) — what you did and why"
                  rows="3"
                  style="
                    width: 100%;
                ...
                <h4
                  v-if="modSelectedEvent.related_events?.length"
                  style="margin: 24px 0 10px; font-size: 13px; font-weight: 600"
                >
                  Other events for this workspace
                </h4>
                <div
                  v-if="modSelectedEvent.related_events?.length"
                ...
                >
                  <div
                    v-for="r in modSelectedEvent.related_events"
                    :key="r.id"
                    class="mod-related-row"
                    @click="openModEvent(r.id)"
                  >
                    <span
                      :class="['mod-sev', modSeverityClass(r.severity)]"
                      style="margin-right: 8px"
                      >{{ r.severity }}</span
                    >
                    <span style="flex: 1; color: var(--gm-text)"
                      >{{ modSourceLabel(r.source) }} —
                      {{ r.operation || "—" }}</span
                    >
        ...
        <template v-if="activeView === 'affiliates'">
          <div class="aff-page">
            <div v-if="affiliateError" class="aff-alert">
              {{ affiliateError }}
            </div>
            ...
            <div class="aff-metrics">
              <div class="metric-card yellow">
                ...
                <div class="metric-value">
                  {{ money(affiliateTotals.available) }}
                </div>
                <div class="metric-sub">
                  {{ affiliateOwing }} affiliate{{
                    affiliateOwing === 1 ? "" : "s"
                  }}
                  awaiting payment
                </div>
              ...
              <div class="metric-card blue">
                ...
                <div class="metric-value">
                  {{ money(affiliateTotals.revenue) }}
                </div>
                <div class="metric-sub">
                  {{ affiliateTotals.sales }} sale{{
                    affiliateTotals.sales === 1 ? "" : "s"
                  }}
            ...
            <div class="section">
              <div class="section-header">
                ...
                <div class="section-actions">
                  <span class="meta-count"
                    >Paid out by us, by hand — nothing here moves money</span
                  >
                  <button
                    class="btn btn-ghost btn-sm"
              ...
              <div class="table-wrap">
                <div
                  v-if="affiliatesLoading && !affiliates.length"
                  class="aff-empty"
                >
                  Loading…
                </div>
                <table v-else-if="affiliates.length">
                  ...
                  <tbody>
                    <tr
                      ...
                      :class="[
                        'aff-row',
                        affiliateOpen?.id === a.id ? 'aff-row-open' : '',
                      ]"
                    >
                      <td>
                        <div class="aff-name">
                      ...
                      <td class="aff-num">
                        <span
                          :class="[
                            'aff-owed',
                            a.available > 0 ? 'aff-owed-due' : '',
                          ]"
                          >{{ money(a.available) }}</span
                        >
                        <div v-if="a.payout_blocked_reason" class="aff-sub">
                          {{ a.payout_blocked_reason }}
                        </div>
                      </td>
                      <td class="aff-num aff-dim">
                        {{ money(a.maturing) }}
                        <div v-if="a.next_matures_at" class="aff-sub">
                          clears {{ a.next_matures_at }}
                        </div>
                      </td>
                      <td class="aff-num aff-dim">
                        {{ money(a.paid) }}
                        <div v-if="a.payouts_count" class="aff-sub">
                          {{ a.payouts_count }} payment{{
                            a.payouts_count === 1 ? "" : "s"
                          }}
                        </div>
                      </td>
                      <td>
                        <template v-if="a.last_payout">
                          <code class="aff-ref">{{
                            a.last_payout.reference
                          }}</code>
                          <div class="aff-sub">
                            {{ money(a.last_payout.amount) }} ·
                            {{ a.last_payout.paid_at }}
                          </div>
                        </template>
                      ...
                      <td class="aff-actions">
                        <button
                          ...
                          :disabled="
                            affiliateBusy === a.id || !!a.payout_blocked_reason
                          "
                          :title="a.payout_blocked_reason || ''"
                          @click="startPayout(a)"
                        >
                          Pay {{ money(a.available) }}
                        </button>
                        <button
                          class="btn btn-ghost btn-sm"
                          @click="openAffiliate(a, 'outstanding')"
                        >
            ...
            <div v-if="affiliateOpen" class="section">
              <div class="section-header">
                ...
                <div class="section-actions">
                  <button
                    ...
                    class="btn btn-ghost btn-sm"
                    @click="
                      downloadStatement(affiliateOpen, { unpaidOnly: true })
                    "
                  >
                    Preview statement
                  </button>
              ...
              <div class="aff-tabs">
                <button
                  ...
                  :class="[
                    'aff-tab',
                    affiliateTab === t.key ? 'aff-tab-active' : '',
                  ]"
              ...
              <div class="table-wrap">
                <div v-if="affiliateDetailLoading" class="aff-empty">
                  Loading…
                </div>
                ...
                <template v-else-if="affiliateTab === 'bank'">
                  <div v-if="affiliateBankLoading" class="aff-empty">
                    Loading…
                  </div>
                  <div v-else-if="!affiliateBank" class="aff-empty">
                    No account on file. The affiliate adds this themselves from
                    their dashboard.
                  </div>
                  <div v-else class="aff-access">
                    <div class="aff-access-row">
                      ...
                      <span
                        :class="[
                          'aff-pill',
                          {
                            verified: 'aff-pill-ok',
                            unverified: 'aff-pill-due',
                            rejected: 'aff-pill-bad',
                          }[affiliateBank.status] ?? 'aff-pill-muted',
                        ]"
                        >{{ affiliateBank.status }}</span
                      >
                      <span v-if="affiliateBank.verified_at" class="aff-sub"
                        >since {{ affiliateBank.verified_at }}</span
                      >
                    </div>
                    <div class="aff-access-row">
                    ...
                    <div class="aff-access-row">
                      ...
                      <span v-if="affiliateBank.bank_code" class="aff-sub"
                        >code {{ affiliateBank.bank_code }}</span
                      >
                    </div>
                    <div class="aff-access-row">
                      ...
                      <code class="aff-code">{{
                        affiliateBankShown
                          ? affiliateBank.account_number
                          : "••••••••"
                      }}</code>
                      <button
                        class="btn btn-ghost btn-xs"
                        @click="affiliateBankShown = !affiliateBankShown"
                      >
                    ...
                    <div class="aff-access-row">
                      ...
                      <span
                        >{{ affiliateBank.payout_currency }} ·
                        {{ affiliateBank.country }}</span
                      >
                    </div>
                    <div
                      v-if="affiliateBank.rejected_reason"
                      class="aff-access-row"
                    >
                      <span class="metric-label">Rejected</span>
                      <span class="aff-sub">{{
                        affiliateBank.rejected_reason
                      }}</span>
                    </div>
                    <div class="aff-access-row">
                      <span class="metric-label">Submitted</span>
                      <span class="aff-sub">{{
                        affiliateBank.submitted_at || "—"
                      }}</span>
                    </div>
                ...
                <template v-else-if="affiliateTab === 'access'">
                  <div class="aff-access">
                    ...
                    <div class="aff-access-row">
                      ...
                      <span class="aff-sub"
                        >public — appears in their links</span
                      >
                    </div>
                    <div class="aff-access-row">
                      ...
                      <code v-if="!affiliateOpen.access_key" class="aff-code"
                        >not issued yet</code
                      >
                      <template v-else>
                        <code class="aff-code">{{
                          affiliateKeyShown
                            ? affiliateOpen.access_key
                            : "•".repeat(23)
                        }}</code>
                        <button
                          class="btn btn-ghost btn-xs"
                          @click="affiliateKeyShown = !affiliateKeyShown"
                        >
                          {{ affiliateKeyShown ? "Hide" : "Reveal" }}
                        </button>
                      </template>
                      <span class="aff-sub"
                        >private — never put this in a link</span
                      >
                    </div>
                    <div class="aff-access-row">
                    ...
                    <div class="aff-access-actions">
                      <button
                        class="btn btn-primary btn-sm"
                        :disabled="!affiliateOpen.access_key"
                        @click="copyAffiliateSignIn(affiliateOpen)"
                      >
                        {{
                          affiliateCopied === `signin-${affiliateOpen.id}`
                            ? "Copied"
                  ...
                  <table v-if="affiliatePayouts.length">
                    <thead>
                      <tr>
                        <th>Reference</th>
                        <th>Sent</th>
                        <th>Covering sales</th>
                        <th class="aff-num">Sales</th>
                        <th class="aff-num">Commission</th>
                        <th class="aff-num">Rate</th>
                        <th class="aff-num">Paid</th>
                        <th>Status</th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr
                        v-for="p in affiliatePayouts"
                        :key="p.id"
                        :class="p.status === 'void' ? 'aff-void' : ''"
                      >
                        <td>
                          <code class="aff-ref">{{ p.reference }}</code>
                          <span
                            v-if="p.status === 'void'"
                            class="aff-pill aff-pill-void"
                            >void</span
                          >
                        </td>
                        <td>{{ p.paid_at || "—" }}</td>
                        ...
                        <td class="aff-num aff-dim">
                          {{
                            p.fx_rate
                              ? Number(p.fx_rate).toLocaleString(undefined, {
                                  maximumFractionDigits: 2,
                                })
                              : "—"
                          }}
                        ...
                        <td class="aff-num">
                          <strong>{{ naira(p.payout_amount) }}</strong>
                        </td>
                        <td>
                          <span
                            :class="[
                              ...
                              {
                                paid: 'aff-pill-ok',
                                processing: 'aff-pill-due',
                                pending: 'aff-pill-due',
                                failed: 'aff-pill-bad',
                                void: 'aff-pill-muted',
                              }[p.status] ?? 'aff-pill-muted',
                            ]"
                            >{{ p.status }}</span
                          >
                          <div v-if="p.payment_reference" class="aff-sub">
                            {{ p.payment_reference }}
                          </div>
                          <div v-if="p.failure_reason" class="aff-sub">
                            {{ p.failure_reason }}
                          </div>
                          <div v-else-if="p.status === 'void'" class="aff-sub">
                            {{ p.void_reason }}
                          </div>
                        </td>
                        <td class="aff-actions">
                          ...
                          <button
                            class="btn btn-ghost btn-xs"
                            @click="
                              downloadStatement(affiliateOpen, { payout: p })
                            "
                          >
                            CSV
                          </button>
                          <button
                            ...
                            @click="voidPayout(affiliateOpen, p)"
                          >
                            Void
                          </button>
                        </td>
                ...
                <template v-else>
                  <table v-if="affiliateConversions.length">
                    <thead>
                      <tr>
                        ...
                        <th>Customer</th>
                        <th>Plan</th>
                        <th class="aff-num">Customer paid</th>
                        <th class="aff-num">Basis</th>
                        <th class="aff-num">Commission</th>
                        <th>Attribution</th>
                        <th>Settled by</th>
                      </tr>
                    ...
                    <tbody>
                      <tr v-for="c in affiliateConversions" :key="c.id">
                        ...
                        <td>{{ c.plan || "—" }}</td>
                        <td class="aff-num aff-dim">
                          {{ money(c.order_amount) }}
                        </td>
                        <td class="aff-num aff-dim">
                          {{ money(c.basis_amount) }}
                        </td>
                        <td class="aff-num">
                        ...
                        <td>
                          <code v-if="c.payout_reference" class="aff-ref">{{
                            c.payout_reference
                          }}</code>
                  ...
                  <div v-else class="aff-empty">
                    {{
                      affiliateTab === "outstanding"
                        ? "Nothing outstanding — everything earned has been paid."
            ...
            <div class="section">
              <div class="section-header">
                ...
                <div class="section-actions">
                  <span class="meta-count"
                    >They never log in — their link is all they get</span
                  >
                </div>
              </div>
              <div class="aff-form">
                <label
                  >Name<input
                    v-model="affiliateForm.name"
                    class="search-input"
                    placeholder="Jane Marketer"
                /></label>
                <label
                  >Email<input
                    v-model="affiliateForm.email"
                    class="search-input"
                ...
                <label>
                  Code
                  <span class="aff-hint"
                    >optional — generated if left blank</span
                  >
                  <input
                    v-model="affiliateForm.code"
                    class="search-input"
                    placeholder="auto"
                  />
                </label>
                <label
                  >Commission %<input
                    v-model.number="affiliateForm.commission_percent"
                    class="search-input"
                    type="number"
                    min="0"
                    max="100"
          ...
          <div
            v-if="payoutStatusDraft"
            class="aff-modal-overlay"
            @click.self="payoutStatusDraft = null"
          >
            <div class="aff-modal">
              <div class="aff-modal-head">
                <div class="section-title">
                  {{ payoutStatusDraft.payout.reference }}
                </div>
              </div>
              <div class="aff-modal-body">
                <div class="aff-modal-total">
                  <div>
                    ...
                    <div class="metric-value">
                      {{ money(payoutStatusDraft.payout.total_amount) }}
                    </div>
                  </div>
                  <div class="aff-modal-total-sub">
                    ...
                    at
                    {{
                      payoutStatusDraft.payout.fx_rate
                        ? Number(
                            payoutStatusDraft.payout.fx_rate
                ...
                <label class="aff-field">
                  Status
                  <select
                    v-model="payoutStatusDraft.status"
                    class="search-input"
                  >
                ...
                <label class="aff-field">
                  Bank reference
                  <span class="aff-hint"
                    >optional — what reconciles this to a statement</span
                  >
                  <input
                    v-model="payoutStatusDraft.payment_reference"
                    class="search-input"
                    placeholder="GTB-99182"
                  />
                </label>
                ...
                <label class="aff-field">
                  Rate actually used
                  <span class="aff-hint"
                    >optional — restates the naira figure with it</span
                  >
                  <input
                    v-model="payoutStatusDraft.fx_rate"
                    class="search-input"
                    type="number"
                    step="0.000001"
                    min="0"
                ...
                <label
                  v-if="payoutStatusDraft.status === 'failed'"
                  class="aff-field"
                >
                  What went wrong
                  <span class="aff-hint">the affiliate sees this</span>
                  <input
                    v-model="payoutStatusDraft.failure_reason"
                    class="search-input"
                    placeholder="Account name mismatch"
                  />
                </label>
              </div>
              <div class="aff-modal-foot">
                <button class="btn btn-ghost" @click="payoutStatusDraft = null">
                  Cancel
                </button>
                <button class="btn btn-primary" @click="savePayoutStatus">
                  Save
                </button>
              </div>
            </div>
          ...
          <div
            v-if="payoutDraft"
            class="aff-modal-overlay"
            @click.self="payoutDraft = null"
          >
            <div class="aff-modal">
              <div class="aff-modal-head">
                <div class="section-title">
                  Record payment to {{ payoutDraft.affiliate.name }}
                </div>
              </div>
              <div class="aff-modal-body">
                <div class="aff-modal-total">
                  <div>
                    ...
                    <div class="metric-value">
                      {{ money(payoutDraft.affiliate.available) }}
                    </div>
                  </div>
                  <div class="aff-modal-total-sub">
                    covering
                    {{
                      payoutDraft.affiliate.available_count ??
                      payoutDraft.affiliate.owed_sales
                    }}
                    matured sale(s)<template
                      v-if="payoutDraft.affiliate.oldest_unpaid"
                      ><br />since
                      {{ payoutDraft.affiliate.oldest_unpaid }}</template
                    >
                  </div>
                </div>

                <p class="aff-modal-note">
                  This records money you have already sent. The sales it covers
                  are fixed now — anything arriving afterwards stays owed and
                  goes on the next payment.
                </p>

                <label class="aff-field">
                  Only sales up to
                  <span class="aff-hint"
                    >optional — leave blank for everything owed</span
                  >
                  <input
                    v-model="payoutDraft.up_to"
                    class="search-input"
                    type="date"
                  />
                </label>
                ...
                <label class="aff-field">
                  ...
                  <input
                    v-model="payoutDraft.note"
                    class="search-input"
                    placeholder="Transfer ref, agreed adjustment…"
                  />
                </label>
              </div>
              <div class="aff-modal-foot">
                <button class="btn btn-ghost" @click="payoutDraft = null">
                  Cancel
                </button>
                <button
                  class="btn btn-pay"
                  :disabled="affiliateBusy === payoutDraft.affiliate.id"
                  @click="confirmPayout"
                >
                  {{
                    affiliateBusy === payoutDraft.affiliate.id
                      ? "Recording…"
                      : "Record payment & download statement"
                  }}
                </button>
              </div>
            </div>
        ...
        <template v-if="activeView === 'plans'">
          <div class="plans-page">
            ...
            <p
              ...
              "
            >
              All available plan tiers, their monthly credit allocations, and
              feature gates.
            </p>
            <div class="plans-grid">
              ...
              >
                ...
                <div class="plan-credits">
                  ...
                  <div class="plan-credits-label">
                    {{
                      plan.key === "free" ? "one-time grant" : "credits / month"
                    }}
                  </div>
                </div>
                <div class="plan-features">
                  <div class="plan-feature">
                    ...
                    ><span class="pf-val">{{
                      plan.channel_limit >= 999 ? "∞" : plan.channel_limit
                    }}</span>
                  </div>
                  <div class="plan-feature">
                    <span class="pf-label">AI image quality</span
                    ><span class="pf-val">{{
                      (plan.ai_image_quality || ["medium"]).join(" + ")
                    }}</span>
                  </div>
                  <div class="plan-feature">
                    ...
                    ><span
                      :class="[
                        'pf-val',
                        plan.watermark ? 'pf-yes-bad' : 'pf-no',
                      ]"
                      >{{ plan.watermark ? "Yes" : "No" }}</span
                    >
                  </div>
                  <div class="plan-feature">
                    <span class="pf-label">Renders / mo</span
                    ><span class="pf-val">{{
                      plan.render_limit >= 9999 ? "∞" : plan.render_limit
                    }}</span>
                  </div>
                </div>
                <div class="plan-cost-row">
                  <div class="plan-cost-item">
                    ...
                    ><span>{{ creditCosts.SCRIPT }} cr</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>Script rewrite</span
                    ><span>{{ creditCosts.SCRIPT }} cr</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>Scene breakdown</span
                    ><span>{{ creditCosts.BREAKDOWN }} cr</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>Stock visual</span
                    ><span>{{ creditCosts.STOCK }} cr / scene</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>AI image (med)</span
                    ><span>{{ creditCosts.AI_MEDIUM }} cr / scene</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>AI image (high)</span
                    ><span>{{ creditCosts.AI_HIGH }} cr / scene</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>TTS voice</span
                    ><span>{{ creditCosts.TTS }} cr / scene</span>
                  </div>
                  <div class="plan-cost-item">
                    <span>TTS regen</span
                    ><span>{{ creditCosts.TTS }} cr / scene</span>
            ...
            <div class="gm-section-title" style="margin-top: 32px">
            ...
            <div class="cost-table">
              ...
              >
                ...
                <div>{{ ex.scenes }}</div>
                <div>
                  <strong>{{ ex.credits }}</strong>
                </div>
                <div style="color: var(--gm-muted)">
                  ${{ (ex.credits * 0.01).toFixed(2) }}
                </div>
              </div>
            </div>
          </div>
        ...
        <template v-if="activeView === 'sfx'">
          <div class="sfx-page">
            ...
            <p
              ...
              "
            >
              Upload royalty-free sound effects users can browse in the editor.
            ...
            <div class="sfx-admin-upload">
              <div class="sfx-admin-row">
                <label class="sfx-admin-field" style="flex: 2">
                  ...
                  <input
                    v-model="sfxUploadForm.name"
                    class="sfx-admin-input"
                    placeholder="e.g. Whoosh Light"
                  />
                </label>
                <label class="sfx-admin-field" style="flex: 1">
                  <span class="sfx-admin-label">Category (optional)</span>
                  <select
                    v-model="sfxUploadForm.category"
                    class="sfx-admin-input"
                  >
                    <option :value="null">— None —</option>
                    <option
                      v-for="c in sfxCategories"
                      :key="c.key"
                      :value="c.key"
                    >
                      {{ c.label }}
                ...
                <label class="sfx-admin-field" style="flex: 1">
                  ...
                  <input
                    v-model="sfxUploadForm.source"
                    class="sfx-admin-input"
                    placeholder="pixabay, mixkit…"
                  />
                </label>
              </div>
              <div class="sfx-admin-row">
                <input
                  ...
                  accept="audio/*"
                  multiple
                  class="sfx-admin-file"
                  @change="onSfxFiles"
                />
                <button
                  class="btn btn-primary btn-sm"
                  :disabled="sfxUploading || !sfxQueuedFiles.length"
                  @click="uploadSfx"
                >
                  {{
                    sfxUploading
                      ? `Uploading ${sfxUploadIndex + 1} / ${
                          sfxQueuedFiles.length
                        }…`
                      : `Upload ${sfxQueuedFiles.length || ""} file${
                          sfxQueuedFiles.length === 1 ? "" : "s"
                        }`
                  }}
                </button>
              </div>
              <div v-if="sfxUploadError" class="sfx-admin-error">
                {{ sfxUploadError }}
              </div>
              <div v-if="sfxQueuedFiles.length" class="sfx-admin-hint">
                Each file will be uploaded with the name above (multiple files:
                filename is used per file, truncated to 60 chars).
              </div>
            </div>
            ...
            <div class="sfx-admin-filters">
              <input
                v-model="sfxFilters.q"
                class="sfx-admin-input"
                placeholder="Search…"
                style="max-width: 280px"
                @input="debouncedSfxSearch"
              />
              <select
                v-model="sfxFilters.category"
                class="sfx-admin-input"
                style="max-width: 200px"
                @change="loadSfxList"
              >
                <option value="">All categories</option>
                <option v-for="c in sfxCategories" :key="c.key" :value="c.key">
                  {{ c.label }}
                </option>
              </select>
              <select
                v-model="sfxFilters.status"
                class="sfx-admin-input"
            ...
            <div
              v-else-if="!sfxList.length"
              class="empty-cell"
              style="padding: 32px"
            >
              No sounds yet. Upload your first one above.
            </div>
            <div v-else class="sfx-admin-table">
              ...
              <div
                v-for="sound in sfxList"
                :key="sound.id"
                class="sfx-admin-tr"
              >
                <button
                  class="sfx-admin-play"
                  :title="sfxPlayingId === sound.id ? 'Stop' : 'Preview'"
                  @click="toggleSfxPreview(sound)"
                >
                ...
                <div style="color: var(--gm-muted); font-size: 12px">
                  {{
                    sound.duration_seconds
                      ? sound.duration_seconds.toFixed(1) + "s"
                      : "—"
                  }}
                </div>
                <div style="color: var(--gm-muted); font-size: 12px">
                ...
                <button
                  class="sfx-admin-delete"
                  title="Delete"
                  @click="deleteSfx(sound)"
                >
                  ✕
                </button>
              </div>
    ...
    <!-- User Detail Panel -->
    <div
      :class="['panel-overlay', panelOpen ? 'open' : '']"
      @click="closePanel"
    ></div>
    <div :class="['user-panel', panelOpen ? 'open' : '']">
      <div v-if="panelLoading" class="gm-spinner-wrap" style="height: 200px">
        Loading user...
      </div>
      <template v-else-if="panelData">
        ...
        <div class="panel-header">
          ...
          <div class="panel-user-info">
            ...
            <div class="panel-badges">
              <span
                :class="[
                  'badge',
                  planBadgeClass(panelData.workspace?.plan_tier),
                ]"
                >{{ panelData.workspace?.plan_tier ?? "no plan" }}</span
              >
              <span
                :class="[
                  'badge',
                  panelData.user?.status === 'active'
                    ? 'badge-green'
                    : 'badge-red',
                ]"
                >{{ panelData.user?.status }}</span
              >
            </div>
          </div>
          <div class="panel-actions">
            <button
              ...
              @click="impersonate(panelData.user.id)"
            >
            ...
            <button
              ...
              :class="[
                'btn btn-sm',
                panelData.workspace.status === 'active'
                  ? 'btn-danger'
                  : 'btn-ghost',
              ]"
              :disabled="suspending"
              @click="
                suspendWorkspace(
                  panelData.workspace.id,
                  panelData.workspace.status
                )
              "
            >
              {{
        ...
        <div class="panel-tabs">
          <button
            ...
            :key="tab"
            :class="['panel-tab', panelTab === tab ? 'active' : '']"
            @click="panelTab = tab"
          >
        ...
        <div class="panel-body">
            ...
            <div class="mini-stats mini-stats-3">
              <div class="mini-stat">
                ...
                <div class="mini-stat-value" style="color: #7c3aed">
                  {{ money(panelData.spend_month_usd) }}
                </div>
              </div>
              <div class="mini-stat">
                <div class="mini-stat-label">Total Spend</div>
                <div class="mini-stat-value">
                  {{ money(panelData.spend_total_usd) }}
                </div>
              </div>
              <div class="mini-stat">
                <div class="mini-stat-label">Projects</div>
                <div class="mini-stat-value">
                  {{ panelData.recent_projects?.length ?? 0 }}+
                </div>
              </div>
              <div class="mini-stat">
                ...
                <div
                  ...
                  style="color: #10b981; font-size: 16px; margin-top: 4px"
                >
                  {{ panelData.storage?.total_human ?? "—" }}
                </div>
              </div>
              <div class="mini-stat">
                <div class="mini-stat-label">Assets</div>
                <div class="mini-stat-value">
                  {{ (panelData.storage?.asset_count ?? 0).toLocaleString() }}
                </div>
              </div>
              <div class="mini-stat">
              ...
              >
                ...
                <div class="mini-stat-value" style="color: #ff6b35">
                  {{ (panelData.credits?.balance ?? 0).toLocaleString() }}
                </div>
                <div
                  style="
                    font-size: 10px;
                ...
                >
                  monthly
                  {{
                    (panelData.credits?.monthly_balance ?? 0).toLocaleString()
                  }}
                  · topup
                  {{ (panelData.credits?.topup_balance ?? 0).toLocaleString() }}
                </div>
              </div>
            </div>
            <div v-if="panelData.workspace" class="ws-card">
              <div class="ws-header">
                ...
                <span
                  ...
                  >{{ panelData.workspace.status }}</span
                >
                <span
                  :class="[
                    'badge',
                  ...
                  >{{ panelData.workspace.plan_tier }}</span
                >
              </div>
              <div class="ws-stats">
                <div class="ws-stat">
                  <div class="ws-stat-val">
                    {{ panelData.usage?.renders_used ?? 0 }}
                  </div>
                  <div class="ws-stat-label">Renders Used</div>
                </div>
                <div class="ws-stat">
                  <div class="ws-stat-val">
                    {{ panelData.usage?.active_channels ?? 0 }}
                  </div>
                ...
                <div class="ws-stat">
                  <div class="ws-stat-val">
                    {{
                      panelData.usage?.api_budget_usd
                        ? Math.round(
                            (panelData.spend_month_usd /
                              panelData.usage.api_budget_usd) *
                              100
                          )
                        : 0
          ...
          <template v-if="panelTab === 'spend'">
              ...
              <div class="chart-bars" style="height: 100px">
                ...
                >
                  <div
                    ...
                    :style="{
                      height: `${Math.round(
                        (Number(day.spend) / panelSpendMax) * 100
                      )}%`,
                    }"
                  ></div>
                </div>
                <div
                  v-if="!panelData.spend_by_day?.length"
                  style="
                    display: flex;
                    align-items: center;
            ...
            <div v-if="panelData.provider_breakdown?.length">
              ...
              <div
                v-for="row in panelData.provider_breakdown"
            ...
            <div
              v-else
              style="color: #6b7280; font-size: 13px; padding: 20px 0"
            >
          ...
          <template v-if="panelTab === 'credits'">
            ...
            <div
              v-if="panelData.ledger_summary?.length"
              style="margin-top: 18px"
            >
              <div class="panel-section-title">Last 30 days · by operation</div>
              <div class="admin-ledger-summary">
                <div
                  v-for="row in panelData.ledger_summary"
                  :key="row.operation"
                  class="admin-ledger-summary-row"
                >
                  <span class="admin-op-tag">{{ row.operation }}</span>
                  <span class="admin-op-credits"
                    >{{ Math.abs(row.credits).toLocaleString() }} cr</span
                  >
                  <span class="admin-op-count"
                    >{{ row.ops }} {{ row.ops === 1 ? "op" : "ops" }}</span
                  >
                </div>
              </div>
            </div>
            ...
            <div
              v-if="panelData.ledger_recent?.length"
              style="margin-top: 18px"
            >
              <div class="panel-section-title">
                Last {{ panelData.ledger_recent.length }} entries
              </div>
                  ...
                  <tr v-for="e in panelData.ledger_recent" :key="e.id">
                    ...
                    <td>
                      <span
                        :class="[
                          'admin-op-tag',
                          e.operation.startsWith('grant:') ? 'grant' : '',
                        ]"
                        >{{ e.operation }}</span
                      >
                    </td>
                    <td>
                      <template v-if="e.project_id"
                        >project #{{ e.project_id
                        }}<template v-if="e.scene_id">
                          · scene #{{ e.scene_id }}</template
                        ></template
                      >
                      <template v-else-if="e.scene_id"
                        >scene #{{ e.scene_id }}</template
                      >
                      <template v-else>—</template>
                    </td>
            ...
            <div v-else class="banner" style="margin-top: 18px">
                ...
                <tr v-for="p in panelData.recent_projects ?? []" :key="p.id">
                  ...
                  <td>
                    <span
                      :class="[
                        'badge',
                        ['ready_for_review', 'published'].includes(p.status)
                          ? 'badge-green'
                          : p.status === 'failed'
                          ? 'badge-red'
                          : 'badge-gray',
      ...
      <div class="adm-modal">
        ...
        <div class="adm-modal-actions">
          <button
            ...
            class="adm-modal-btn adm-modal-btn-ghost"
            @click="modalCancel"
          >
            Cancel
          </button>
          <button
            :class="[
              'adm-modal-btn',
              modal.danger ? 'adm-modal-btn-danger' : 'adm-modal-btn-primary',
            ]"
            @click="modalConfirm"
          >
            {{ modal.confirmLabel }}
...
.mod-counter-warn .mod-counter-value {
  color: #fca5a5;
}
.mod-filters {
  display: flex;
...
.sev-high {
  background: rgba(251, 113, 53, 0.2);
  color: #fdba74;
}
.sev-crit {
...
.mod-detail-row > span:first-child {
  color: #6b7280;
  font-size: 12px;
}
.mod-related-row {
...
.sfx-admin-input {
  ...
  border-radius: 6px;
  padding: 7px 10px;
  color: var(--gm-fg, #ececf3);
  font-size: 13px;
  font-family: inherit;
  outline: none;
  min-width: 0;
}
.sfx-admin-input:focus {
  border-color: var(--gm-accent, #ff6b35);
}
.sfx-admin-file {
  color: var(--gm-muted);
  font-size: 12px;
...
.sfx-admin-tr {
  display: grid;
  grid-template-columns: 40px 2fr 1fr 80px 1fr 110px 40px;
  gap: 10px;
  align-items: center;
  padding: 10px 14px;
}
...
.sfx-admin-play:hover {
  background: var(--gm-accent, #ff6b35);
  color: #fff;
  border-color: var(--gm-accent, #ff6b35);
}
.sfx-inline {
...
.plan-cost-item span:last-child {
  color: #c9cad4;
  font-weight: 500;
  font-family: monospace;
}
...
.cw-members td {
  ...
  border-bottom: 1px solid var(--color-border, #23232d);
  font-size: 12.5px;
}
.cw-members tr:last-child td {
  border-bottom: none;
}
.cwm-mail {
  width: 34%;
...
.mail-input {
  width: 100%;
  padding: 9px 12px;
  background: var(--gm-card, var(--color-bg-card));
  border: 1px solid var(--gm-border, var(--color-border));
  border-radius: 8px;
  color: inherit;
  font-family: inherit;
  font-size: 13px;
...
.gm-table tr.gm-row-click {
  cursor: pointer;
}
.gm-table .mail-history-detail {
  background: #12151b;
}
.gm-empty {
  background: #161920;
...
.mail-recip.is-out .mail-recip-email {
...
.mail-dossier {
  margin-top: 8px;
  padding: 10px 12px;
  background: var(--gm-card, rgba(255, 255, 255, 0.03));
  border: 1px solid var(--gm-border, #2a2a36);
  border-radius: 8px;
  font-size: 11px;
  color: var(--gm-muted);
}
...
.mail-history-detail {
  background: var(--gm-card, rgba(255, 255, 255, 0.02));
...
.aff-field .search-input,
.aff-form .search-input {
  width: 100%;
