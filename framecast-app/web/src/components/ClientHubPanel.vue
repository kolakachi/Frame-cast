  ...
  <section class="hub" aria-label="Client workspace management">
    ...
    <template v-else-if="data">
      ...
      <div v-if="tab==='Overview'" class="panel">
        ...
        <div v-if="data.can_produce && data.workspace.status==='active'" class="actions"><button @click="open('/dashboard')">Create or continue a video</button><button @click="open('/assets')">Open asset library</button><button @click="open('/ugc-ads')">Create UGC</button><template v-if="auth.user?.is_internal"><button @click="open('/from-my-footage')">Use my footage</button></template></div>
