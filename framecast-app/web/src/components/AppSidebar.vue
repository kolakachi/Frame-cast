...
const isAdmin = computed(() =>
  ["super_admin", "platform_admin"].includes(props.user?.role)
);
  ...
  <nav :class="['sidebar', isCollapsed ? 'collapsed' : '']">
    ...
    >
      ...
      <svg
        class="ws-caret"
        :style="{
          transform: showWsPopover ? 'rotate(180deg)' : 'rotate(0deg)',
        }"
        width="12"
        height="12"
        fill="none"
    ...
    <div v-if="showWsPopover" class="ws-popover" @click.stop>
      ...
      <div v-if="workspaceStore.usage" class="ws-popover-usage">
        <div class="ws-usage-row">
          ...
          <span class="ws-usage-val"
            >{{ workspaceStore.usage.renders_used }}/{{
              workspaceStore.usage.render_limit
            }}</span
          >
        </div>
      ...
      <button
        class="ws-popover-action ws-popover-disabled"
        type="button"
        disabled
        title="Multi-workspace support coming soon"
      >
        <svg
          width="13"
          height="13"
    ...
    <div class="sidebar-nav">
      ...
      <button
        :class="['nav-item', activePage === 'videos' ? 'active' : '']"
        data-tooltip="All Videos"
        type="button"
        @click="nav('videos')"
      >
        <svg
      ...
      <button
        v-if="isAdmin"
        :class="['nav-item', activePage === 'admin' ? 'active' : '']"
        data-tooltip="God Mode"
        type="button"
        @click="nav('admin')"
      >
        <svg
    ...
    <div class="sidebar-bottom">
      ...
      <a
        ...
        title="Join the WyvStudio community on Facebook"
      >
      ...
      <div v-if="showUserPopover" class="user-popover" @click.stop>
        ...
        <button
          class="user-popover-action danger"
          type="button"
          @click="emit('logout')"
        >
          Log out
...
.ws-popover-action:hover:not(:disabled) {
  background: rgba(255, 255, 255, 0.05);
  color: var(--color-text-primary);
}
.ws-popover-disabled {
...
.user-popover-action.danger:hover {
  color: #fca5a5;
}
