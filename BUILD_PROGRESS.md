...
- [ ] Monorepo initialised — `/api` (Laravel 11) and `/web` (Vue 3 + Vite)
...
- [ ] Vue SPA — router, Pinia auth store, Axios interceptor with token refresh
...
- [ ] Scene regeneration endpoint — `scene_rewrite` prompt, respects `locked_fields_json`
...
- [ ] Translation job — calls translation adapter per scene, title, CTA, overlays
...
- [ ] All rendered outputs pass aspect ratio, safe-area captions, watermark rules
